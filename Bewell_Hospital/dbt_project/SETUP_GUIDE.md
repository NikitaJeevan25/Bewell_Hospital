...
COPY INTO hospital_management_data
FROM @hospital_stage/hospital_management_data.csv
FILE_FORMAT = (
    TYPE = 'CSV',
    SKIP_HEADER = 1,
    FIELD_DELIMITER = ','
...
-- Verify
SELECT COUNT(*) FROM hospital_management_data;
```
...
FROM BEWELL_HOSPITAL_DB.psa_hospital.psa_hospital_admission

UNION ALL

SELECT 'Staging', COUNT(*) 
FROM BEWELL_HOSPITAL_DB.stg_hospital.stg_hospital_admission
