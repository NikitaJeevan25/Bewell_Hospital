...
standardized AS (
    SELECT
        ...
        TRY_TO_DATE(
            CASE 
                WHEN Admission_Date LIKE '%/%' THEN REPLACE(Admission_Date, '/', '-')
                ELSE Admission_Date
            END,
            'YYYY-MM-DD'
        ) AS Admission_Date_Std,
        
        TRY_TO_DATE(
            CASE 
                WHEN Discharge_Date LIKE '%/%' THEN REPLACE(Discharge_Date, '/', '-')
                ELSE Discharge_Date
            END,
            'YYYY-MM-DD'
        ) AS Discharge_Date_Std,
...
data_quality_flags AS (
    SELECT
        *,
        ...
        -- Data Quality Flags
        CASE WHEN Age_Std IS NULL AND Age IS NOT NULL AND (Age < 0 OR Age > {{ var('max_age') }}) THEN 1 ELSE 0 END AS is_invalid_age,
